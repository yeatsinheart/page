...
class GradientSetting {
  ...
  final List<ColorSetting> colors;
  ...
  GradientSetting._({required this.type, required List<ColorSetting> this.colors, this.stops, this.begin, this.end, this.center, this.radius, this.startAngle, this.endAngle});

  factory GradientSetting.linear({required List<ColorSetting> colors, List<double>? stops = const [0, 1], List<double> begin = const [-1, -1], List<double> end = const [1, 1]}) {
  ...
  factory GradientSetting.radial({required List<ColorSetting> colors, List<double>? stops = const [0, 1], List<double> center = const [0, 0], double radius = 0.5}) {
    if (colors.length < 2) throw ArgumentError("At least two colors required");
  ...
  factory GradientSetting.sweep({
    required List<ColorSetting> colors,
    List<double>? stops = const [0, 1],
    List<double> center = const [0, 0],
    double startAngle = 0,
