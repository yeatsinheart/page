...
export async function openby(pop_path, view_path, params, name, event) {
    if (pop_path) {
        const async_view = await view_get_by_path(pop_path)();
        const async_child_view = await view_get_by_path(view_path)();
        push_to_list(pop_path + ">" + view_path, async_view, params, event, async_child_view);
    } else {
        show(view_path, params, name, event);
    }
}

export async function show(view_path, params, name, event) {
